...
//static void    printPairTree(Pair *head)
//{
//    if (head == NULL)
//    {
//        return;
...
//}
//
//static void  printVector(const std::vector<Pair *> &v)
//{
...
std::deque<Pair *>::iterator   PmergeMe::binarySearch(std::deque<Pair *> &main_chain, std::deque<Pair *>::size_type start, int target)
{
    int left = start;
    int right = main_chain.size() - 1;
    int mid = (left + right) / 2;
    ...
    if (static_cast<std::deque<Pair *>::size_type>(left) == main_chain.size())
    {
        return main_chain.end();
    }
...
std::vector<Pair *>::size_type   PmergeMe::insertOdd(std::vector<Pair *> &main_chain, Pair *odd_man)
{
    std::vector<Pair *>::iterator   pos = binarySearch(main_chain, 0, odd_man->winner);
    Pair    *odd_to_insert = odd_man->w_prev ? odd_man->w_prev : odd_man;
    std::vector<Pair *>::size_type   idx_to_return = pos == main_chain.end() ? main_chain.size() - 1 : pos - main_chain.begin();
    main_chain.insert(pos, odd_to_insert);
    return idx_to_return;
}

std::deque<Pair *>::size_type   PmergeMe::insertOdd(std::deque<Pair *> &main_chain, Pair *odd_man)
{
    std::deque<Pair *>::iterator   pos = binarySearch(main_chain, 0, odd_man->winner);
    Pair    *odd_to_insert = odd_man->w_prev ? odd_man->w_prev : odd_man;
    std::deque<Pair *>::size_type   idx_to_return = pos == main_chain.end() ? main_chain.size() - 1 : pos - main_chain.begin();
    main_chain.insert(pos, odd_to_insert);
    return idx_to_return;
}
...
void    PmergeMe::insertInRange(std::deque<Pair *> &main_chain, std::deque<Pair *>::size_type start, std::deque<Pair *>::size_type count, Pair *odd_man = NULL)
{
    std::deque<Pair *>::size_type  range = start + count;

    std::set<Pair *>    inserted;
    Pair    *loser_to_insert = NULL;
    for (std::deque<Pair *>::size_type i = start; i < range; ++i)
    {
        // 바로 다음 칸에 들어간 경우가 아니라 그냥 range 안에 있기만 해도 안됨.
        // 하아아 이런 짜치는 방법 밖에는 없다는 말인가
        if (inserted.find(main_chain[i]) != inserted.end() || odd_man == main_chain[i])
...
void    PmergeMe::insertLosers(std::vector<Pair *> &main_chain, std::vector<Pair *>::size_type target_size)
{
    ...
    if (odd_man)
    {
        std::vector<Pair *>::size_type   pos = insertOdd(main_chain, odd_man);
        if (pos < before)
        {
            ++before;
        }
        odd_man = main_chain[pos];
    }
    count = before;
...
void    PmergeMe::insertLosers(std::deque<Pair *> &main_chain, std::deque<Pair *>::size_type target_size)
{
    if (main_chain.size() >= target_size)
    {
        return;
    ...
    int k = 0;
    std::deque<Pair *>::size_type  before = main_chain.size();
    std::deque<Pair *>::size_type  count = 0;
    for (std::deque<Pair *>::size_type i = main_chain.size(); i-- > 0; )
    {
        std::deque<Pair *>::size_type  revised_idx = main_chain.size() - i;
        if (revised_idx == (1 << k))
        {
            count = before - i;
            insertInRange(main_chain, i, count);
    ...
    if (odd_man)
    {
        std::deque<Pair *>::size_type   pos = insertOdd(main_chain, odd_man);
        if (pos < before)
        {
            ++before;
        }
...
Pair    *PmergeMe::haveTournament(std::deque<Pair *> &players_in_pair)
{
    if (players_in_pair.size() == 1)
    {
        return players_in_pair.front();
    ...
    std::deque<Pair *> next_pairs;
    std::deque<Pair *>::size_type  count = players_in_pair.size() / 2 * 2;
    ...
    for (std::deque<Pair *>::size_type i = 0; i < count; i += 2)
    {
        next_pairs.push_back(generateRankedPair(players_in_pair[i], players_in_pair[i + 1]));
        next_pairs.back()->odd = oddMan;
    }
...
void    PmergeMe::rank(std::deque<int> &players)
{
    std::deque<Pair *>   initial_pairs;
    std::deque<int>::size_type players_size = players.size();
    ...
    if (players_size > 1)
    {
        ...
        for (std::deque<int>::size_type i = 0; i < players_size - 1; i += 2)
        {
            Pair    *new_pair = generateRankedPair(players[i], players[i + 1]);
            new_pair->odd = odd_man;
            initial_pairs.push_back(new_pair);
        ...
        std::deque<Pair *>::size_type  j = 0;
        for (std::deque<Pair *>::size_type i = players_size; i-- > 0; )
        {
            players[j] = main_chain[i]->winner;
            ++j;
        }
